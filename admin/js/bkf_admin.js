...
async function bkfCheckSlug() {
  ...
  const res = await apiGet('api/bkf_admin.php', { action: 'check_slug', slug });
  if (res.ok) {
...
async function bkfCreateForm() {
  ...
  const res = await apiPost('api/bkf_admin.php', {
...
async function bkfLoadFormList() {
  const res   = await apiGet('api/bkf_admin.php', { action: 'list_forms' });
...
async function bkfDeleteForm(formId, title, slug) {
  ...
  const res = await apiPost('api/bkf_admin.php', { action: 'delete_form', id: formId });
  if (res.ok) {
...
async function bkfOpenDetail(formId) {
  bkfCurrentFormId = formId;
  const res = await apiGet('api/bkf_admin.php', { action: 'get_form', id: formId });
...
async function bkfLoadFields() {
  const res = await apiGet('api/bkf_admin.php', { action: 'list_fields', form_id: bkfCurrentFormId });
...
async function bkfSaveFieldOrder() {
  ...
  await apiPost('api/bkf_admin.php', { action: 'sort_fields', orders: JSON.stringify(orders) });
}

// 필드 노출 토글
async function bkfToggleFieldVisible(id, cb) {
  const res = await apiPost('api/bkf_admin.php', { action: 'toggle_field_visible', id, is_visible: cb.checked ? 1 : 0 });
...
async function bkfDeleteField(id) {
  ...
  const res = await apiPost('api/bkf_admin.php', { action: 'delete_field', id, form_id: bkfCurrentFormId });
...
async function bkfSaveField() {
  ...
  const res = await apiPost('api/bkf_admin.php', data);
  if (res.ok) {
...
async function bkfLoadSteps() {
  const res = await apiGet('api/bkf_admin.php', { action: 'list_steps', form_id: bkfCurrentFormId });
  if (!res.ok) return;
...
async function bkfRemoveStep(id, btn) {
  ...
  if (id > 0) {
    const res = await apiPost('api/bkf_admin.php', { action: 'toggle_step', id, is_active: 0 });
    if (!res.ok) { showToast(res.msg || '오류', 'error'); return; }
  }
  row.remove();
...
async function bkfLoadQuotaStores() {
  ...
  if (!sel) return;
  try {
    const res = await apiGet('api_front/bkf_public.php', { action: 'get_stores', slug: bkfCurrentFormData.slug || '' });
    const stores = res.data || [];
    sel.innerHTML = '<option value="">지점 없음 (공통)</option>' +
      stores.map(s => `<option value="${s.id}">${escHtml(s.store_name || s.branch_name || '')}</option>`).join('');
  } catch (e) {
    sel.innerHTML = '<option value="">지점 없음 (공통)</option>';
  }
}
...
async function bkfLoadQuota() {
  ...
  const res = await apiGet('api/bkf_admin.php', params);
...
async function bkfRemoveSlotRow(btn, quotaId) {
  if (quotaId > 0) {
    const res = await apiPost('api/bkf_admin.php', { action: 'delete_quota', id: quotaId });
...
async function bkfLoadManagers() {
  const res   = await apiGet('api/bkf_admin.php', { action: 'list_managers', form_id: bkfCurrentFormId });
...
async function bkfDeleteManager(id) {
  ...
  const res = await apiPost('api/bkf_admin.php', { action: 'delete_manager', id, form_id: bkfCurrentFormId });
...
async function bkfLoadRecords(page = 1) {
  ...
  const res = await apiGet('api/bkf_admin.php', {
    action: 'list_records',
    form_id: bkfCurrentFormId,
...
async function bkfOpenRecordModal(id) {
  const res = await apiGet('api/bkf_admin.php', { action: 'get_record', form_id: bkfCurrentFormId, id });
  if (!res.ok) { showToast(res.msg || '오류', 'error'); return; }
  ...
  try {
    const sRes = await apiGet('api_front/bkf_public.php', { action: 'get_stores', slug: bkfCurrentFormData.slug || '' });
    (sRes.data || []).forEach(s => {
      const opt = document.createElement('option');
      opt.value = s.id;
      opt.textContent = s.store_name || s.branch_name || '';
