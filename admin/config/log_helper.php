 ...
 * 모든 API에서 require_once 로 포함하여 사용
 */
...
function logLogin(PDO $pdo, string $userType, ?string $userId, ?string $username, string $result, string $failReason = ''): void {
    try {
        $pdo->prepare("
            INSERT INTO logs_login (user_type, user_id, username, ip, result, fail_reason)
            VALUES (?, ?, ?, ?, ?, ?)
...
function logAdminAction(PDO $pdo, string $action, string $targetTable = '', string $targetId = '', array $before = [], array $after = []): void {
    ...
    try {
        $pdo->prepare("
            INSERT INTO logs_admin_action (admin_id, admin_name, action, target_table, target_id, before_data, after_data, ip)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ")->execute([
            $admin['id'],
            $admin['name'],
...
function logEmail(PDO $pdo, string $toEmail, string $subject, string $template, string $status, string $errorMsg = ''): void {
    try {
        $pdo->prepare("
            INSERT INTO logs_email (to_email, subject, template, status, error_msg)
            VALUES (?, ?, ?, ?, ?)
...
function logError(PDO $pdo, string $message, string $level = 'error', string $code = '', string $file = '', int $line = 0, string $stackTrace = ''): void {
    try {
        $pdo->prepare("
            INSERT INTO logs_error (level, code, message, file, line, stack_trace, ip)
            VALUES (?, ?, ?, ?, ?, ?, ?)
