...
$subject = isset($body['subject']) ? trim($body['subject']) : '신청 완료 안내';
...
/* ── 사이트 타이틀 ── */
$_siteRow  = getDB()->query("SELECT title, copyright FROM homepage_info WHERE id=1 LIMIT 1")->fetch();
$siteName  = $_siteRow ? $_siteRow['title'] : 'PureBlue';
$siteCopy  = ($_siteRow && $_siteRow['copyright']) ? $_siteRow['copyright'] : ('© ' . date('Y') . ' ' . $siteName);

/* ── 발신자 정보 ── */
$fromName  = $siteName . ' 고객센터';
...
$htmlBody = '<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"></head><body style="font-family:\'Noto Sans KR\',sans-serif;background:#f8fafc;margin:0;padding:0;">'
  ...
  .'<div style="font-family:Montserrat,sans-serif;font-weight:900;font-size:1.3rem;color:#fff;letter-spacing:-0.5px;">'.htmlspecialchars($siteName).'</div>'
  .'</div>'
