/* ════════════════════════════════════════════════
  ...
  function rsvRenderTimeSlotStep(times,merged,nextBtn){
    ...
    times.forEach(function(t){
      ...
      html+='<button type="button" onclick="'+(isFull?'':'rsvSelectTimeSlot(\''+t+'\')')+'" '
        +'style="padding:10px 18px;border-radius:10px;border:2px solid '+(isSel?'var(--color-base)':'var(--g2)')+';background:'+(isSel?'var(--color-base)':isFull?'var(--off)':'#fff')+';color:'+(isSel?'#fff':isFull?'var(--g3)':'var(--ink3)')+';font-size:.84rem;font-weight:700;cursor:'+(isFull?'default':'pointer')+';font-family:inherit;display:flex;flex-direction:column;align-items:center;gap:3px;"'+(isSel?' class="sel-t"':'')+'>'
  ...
  function rsvRenderBranchTimeStep(settings,sub,wrap,nextBtn){
    ...
    for(var h=startH;h<endH;h++){
      ...
      html+='<button type="button" onclick="rsvSelectTimeSlot(\''+val+'\')" style="padding:10px 18px;border-radius:10px;border:2px solid '+(isSel?'var(--color-base)':'var(--g2)')+';background:'+(isSel?'var(--color-base)':'#fff')+';color:'+(isSel?'#fff':'var(--ink3)')+';font-size:.84rem;font-weight:700;cursor:pointer;font-family:inherit;"'+(isSel?' class="sel-t"':'')+'>'+rsvEsc(label)+'</button>';
    }
    html+='</div>';
    wrap.innerHTML=html;
    if(nextBtn) nextBtn.disabled=!_rsv.time;
    ...
    .then(function(res){
      if(res.ok){
        ...
        if(info){
          ...
          var resNumHtml=res.res_number?'<div class="rsv-complete-row" style="border-bottom:1px solid var(--g2);padding-bottom:12px;margin-bottom:4px;"><span style="color:var(--g5)">예약번호</span><span style="font-weight:800;color:var(--color-base);font-size:1rem;">'+rsvEsc(res.res_number)+'</span></div>':'';
