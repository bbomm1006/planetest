...
function sendFrontEml() {
  ...
  if (_frontEmlType === 'inq') {
    var last = window._lastInqData || {};
    subj = '[퓨어블루] ' + (last.name || '') + '님 상담 신청 완료 안내';
    body = '안녕하세요, ' + (last.name || '') + '님.\n\n퓨어블루 상담 신청이 정상적으로 접수되었습니다.\n\n'
    ...
    var last = window._lastComboData || {};
    subj = '[퓨어블루] ' + (last.name || '') + '님 결합 신청 완료 안내';
    body = '안녕하세요, ' + (last.name || '') + '님.\n\n퓨어블루 결합 할인 신청이 정상적으로 접수되었습니다.\n\n'
    ...
    var prods = window._cmpProds || [];
    subj = '[퓨어블루] 제품 비교 내역 안내';
    body = '안녕하세요.\n\n퓨어블루 제품 비교 내역을 안내드립니다.\n\n'
      + prods.map(function(p) {
          return '■ ' + p.name + ' (' + p.model + ')\n  월 렌탈: ' + (+p.priceMonthly || 0).toLocaleString('ko-KR')
            + '원/월\n  특징: ' + (p.features || []).join(', ');
        }).join('\n\n')
      + '\n\n감사합니다.\n\n퓨어블루 고객센터';
  }
  _doSendEml(addr, subj, body);
}
